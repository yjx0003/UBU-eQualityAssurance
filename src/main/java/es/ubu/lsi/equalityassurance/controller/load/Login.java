...
import es.ubu.lsi.moodlerestapi.webservice.api.tool.mobile.ToolMobileGetPublicConfig;
...
public class Login {
	...
	private void loginWithWebView(String host, Window owner) {
		CompletableFuture.runAsync(() -> {
			Stage popup = UtilFXML.createStage(owner, Modality.WINDOW_MODAL);
				...
				webView.getEngine()
						.executeScript(
								"for(const a of document.forms) for (const o of a.getElementsByTagName('INPUT'))'text'==o.type&&!o.hidden&&(o.value ='"
										+ UtilString.escapeJavaScriptText(username)
										+ "'),'password'==o.type&&!o.hidden&&(o.value='"
										+ UtilString.escapeJavaScriptText(password) + "');");
			}
