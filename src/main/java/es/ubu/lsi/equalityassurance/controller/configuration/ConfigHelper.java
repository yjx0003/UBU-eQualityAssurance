package es.ubu.lsi.equalityassurance.controller.configuration;
...
public class ConfigHelper {
	...
	public static void initialize(String path) {
		...
		try {
			if (!file.isFile() && !file.createNewFile()) {
				log.warn("No se ha podido crear el fichero properties, intentando generar uno nuevo: {} ", path);
			...
			log.error("No se ha podido crear el fichero properties: {} ", path);
			properties = new JSONObject();
		}
	...
	public static void save(String path) {
		...
		try (FileWriter file = new FileWriter(path)) {
			...
			log.error("No se ha podido guardar el fichero {}", path);
		}
	}
