...
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseGetCoursesByField;
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseGetEnrolledCoursesByTimelineClassification;
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseGetEnrolledCoursesByTimelineClassification.Classification;
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseGetRecentCourses;
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseGetUserAdministrationOptions;
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseGetUserNavigationOptions;
import es.ubu.lsi.moodlerestapi.webservice.api.core.course.CoreCourseSearchCourses;
import es.ubu.lsi.moodlerestapi.webservice.api.core.enrol.CoreEnrolGetUsersCourses;
...
public class PopulateCourse {
	...
	public List<Course> createCourses(int userid) {
		try {
			JSONArray jsonArray = UtilResponse.getJSONArrayResponse(webService, new CoreEnrolGetUsersCourses(userid));
	...
	public Course userCourses(JSONObject jsonObject) {
		...
		course.setEnablecompletion(jsonObject.optBoolean(Constants.ENABLECOMPLETION));
	...
	public Pair<Integer, List<Course>> searchCourse(String value) {
		...
		try {
			...
			JSONObject jsonObject = UtilResponse.getJSONObjectResponse(webService, coreCourseSearchCourses);
	...
	public void createCourseAdministrationOptions(Collection<Integer> courseids) {
		...
		try {
			...
			jsonArray = UtilResponse.getJSONObjectResponse(webService, new CoreCourseGetUserNavigationOptions(courseids))
					.getJSONArray(Constants.COURSES);
			...
			jsonArray = UtilResponse.getJSONObjectResponse(webService, coreCourseGetCoursesByField)
					.getJSONArray(Constants.COURSES);
